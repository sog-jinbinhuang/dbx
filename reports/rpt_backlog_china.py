...
report.py
=========
Reads from FCT_GLOBAL_BACKLOG and renders the Excel workbook.
...
class Loader:
    """Connects to Databricks and runs queries against FCT_GLOBAL_BACKLOG /
    FCT_GLOBAL_NEW_ORDERS."""
    ...
    def load_new_orders(self) -> pd.DataFrame:
        ...
        df = self._query("""
            ...
            WHERE DATABASE IN ('CHINA','WEIFENG')
        """)
        print(f"    {len(df):,} rows")
        ...
        df = df.sort_values(["DATABASE", "ORDER_DATE"],
    ...
    def load(self) -> pd.DataFrame:
        ...
        cur = self._conn.cursor()
        cur.execute("SELECT * FROM FCT_GLOBAL_BACKLOG WHERE DATABASE IN ('CHINA','WEIFENG')")
        cols = [d[0].upper() for d in cur.description]
        rows = cur.fetchall()
        cur.close()
...
class ReportBuilder:
    ...
    def _sorted_pivot_values(self) -> list[str]:
        ...
        return sorted(vals, key=lambda v: (v == "NEW BUSINESS", v))
...
SEGMENT_CONFIG = ReportConfig(
    ...
    show_rep_col = True,
    col1_label   = "Segment",
    output_file  = os.path.join(_DIR, f"sales_report_china_by_product_segment_{_DATE_SUFFIX}.xlsx"),
)
...
REP_CONFIG = ReportConfig(
    ...
    output_file  = os.path.join(_DIR, f"sales_report_china_by_sales_rep_{_DATE_SUFFIX}.xlsx"),
)
...
def build_attachments() -> list[tuple[str, bytes]]:
    ...
    it, returns both as attachments -- same pattern as rpt_backlog.py."""
    import copy
    import io
    ...
    for config_template, filename in [
        (SEGMENT_CONFIG, f"sales_report_china_by_product_segment_{_DATE_SUFFIX}.xlsx"),
        (REP_CONFIG, f"sales_report_china_by_sales_rep_{_DATE_SUFFIX}.xlsx"),
    ]:
        buf = io.BytesIO()
